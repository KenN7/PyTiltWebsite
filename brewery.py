...
def main():
    ...
    if request.args:
        ...
        origgravity = mean([d.gravity+4 for d in period[:100]])/1000
        finalgravity = mean([d.gravity+4 for d in period[-100:]])/1000
        ...
        # i=0
        # for d in period[::res]:
        #     datalist.append((d.time.timestamp(),mean([d.temp for d in period[i:i+res]]),mean([d.gravity+4 for d in period[i:i+res]])))
        #     i+=res
    else:
        period =  Tilt.select()
        res = int(len(period)/max_points)
        ...
        # for d in period[::res]:
        #     datalist.append((d.time.timestamp(),d.temp,d.gravity+4))
        origgravity = 0
        finalgravity = 0
        abv = 0
        apparent_att = 0
